...
def get_all_tokens_batch_logps(
    logits: "torch.Tensor", labels: "torch.Tensor", label_pad_token_id: int = IGNORE_INDEX
) -> Tuple["torch.Tensor", "torch.Tensor"]:
    r"""
    Computes the log probabilities of the given labels under the given logits.
    ...
    per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    per_token_p = torch.gather(logits.softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    # return (per_token_logps * loss_mask).sum(-1), loss_mask.sum(-1), per_token_logps.clone().detach(), loss_mask.clone().detach()
    return (per_token_logps * loss_mask).sum(-1), loss_mask.sum(-1), per_token_p.clone().detach(), loss_mask.clone().detach()