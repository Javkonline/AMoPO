...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.is_caldpo = finetuning_args.is_caldpo
    ...
    def compute_preference_loss(
        ...
        if not self.finetuning_args.use_ref_model:
            ...
            if self.is_caldpo:
                # pdb.set_trace()
                beta = torch.tensor(self.beta).to(chosen_rewards.device)
                cal_losses = F.mse_loss(chosen_rewards, 0.5*beta)+ F.mse_loss(rejected_rewards, -0.5*beta)
                # print("caldpo")
                losses += cal_losses
